...
def runInterpolation(install_path, imgPath1, imgPath2, outPath):
	...
	executable 	= "python3 " + os.path.join(install_path, "run.py") + " --pretrained_model_checkpoint_path=/home/Workspace/install/ckpt/CyclicGen_large/model"
	command 	= executable + " --first='" +  imgPath1 + "' --second='" + imgPath2 + "'  --out='" + outPath + "' 2> /dev/null"
	os.system(command)

...
def getArgumentsThroughFile(filePath):
	
	fileHandle = open(filePath, 'r')
	lines = fileHandle.readlines()
	fileHandle.close()
	
	fileArguments = {"img1" : lines[0].strip(), "img2" : lines[1].strip(), "out" : lines[2].strip()}
	return fileArguments

if __name__ == "__main__":
	
	fileArguments = getArgumentsThroughFile("custom_file_args.txt")
	
	model = networks.__dict__[args.netName](channel=args.channels, filter_size = args.filter_size, timestep=args.time_step, training=False)

	if args.use_cuda:
		model = model.cuda()

	args.SAVED_MODEL = '/home/Workspace/install/model_weights/best.pth'
	...
	img1 = imread(fileArguments["img1"])
	img2 = imread(fileArguments["img2"])
	...
	X0 =  torch.from_numpy( np.transpose(img1, (2,0,1)).astype("float32")/ 255.0).type(dtype)
	X1 =  torch.from_numpy( np.transpose(img2, (2,0,1)).astype("float32")/ 255.0).type(dtype)
	...
	y_ = [np.transpose(255.0 * item.clip(0,1.0)[0, :, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth], (1, 2, 0)) for item in y_]
	...
	for item, time_offset in zip(y_, time_offsets):
		...
		output = np.round(item).astype(np.uint8)
		output = imresize(output, resolution)
		count = count + 1
		imsave(fileArguments["out"] + "-" + str(int(time_offset * 100)) + ".png", output)
